...
from timeseries.forecast.utils import TimeSeriesData


class ForexGBPUSDData(TimeSeriesData):

    def __init__(self):
        super().__init__("Forex Currencies Rates", "GBP/USD")
...
class WebsiteVisitsData(TimeSeriesData):
    ...
    def __init__(self):
        super().__init__("Website Visits", "Page Loads")
...
class Covid19Data(TimeSeriesData):
    ...
    def __init__(self):
        super().__init__("Covid-19", "Argentina")
    ...
    def __set_competition_params__(self):
        ...
        val_datetime = datetime(2021, 1, 1)
...
class WindSpeedData(TimeSeriesData):

    def __init__(self):
        super().__init__("Weather", "Wind Speed")

    def __load__(self):
        weather_data = pd.read_csv(
            os.path.join(self.dirpath, "weatherHistory.csv")
        )
        weather_data["Formatted Date"] = pd.to_datetime(
            weather_data["Formatted Date"].apply(lambda s: s[:-10])
        )
        weather_data = weather_data[
            ~weather_data.duplicated(subset="Formatted Date", keep=False)
        ]
        weather_data.set_index("Formatted Date", inplace=True)
        self.ts = weather_data["Wind Speed (km/h)"].sort_index()[:datetime(2016,11,1)].copy()
        del weather_data

    def __set_competition_params__(self):
        train_datetime = datetime(2006, 2, 1)
        val_datetime = datetime(2012, 1, 1)
        test_datetime = datetime(2014, 1, 1)
        test_end_datetime = self.ts.index[-1] + timedelta(hours=1)
        self.train_interval = tss.Interval(self.ts, train_datetime,
                                           val_datetime)
        # empty test interval
        self.val_interval = tss.Interval(self.ts, val_datetime, test_end_datetime)
        self.test_interval = tss.Interval(self.ts, test_end_datetime - timedelta(hours=1),
                                          test_end_datetime)
        # val and test intervals
#         self.val_interval = tss.Interval(self.ts, val_datetime, test_datetime)
#         self.test_interval = tss.Interval(self.ts, test_end_datetime,
#                                           test_datetime)
        self.pred_steps = 24  # 24h
        self.pred_jump = 25  # 25h
